...
class OllamaChatbot:
    ...
    def load_files(self, file_paths: List[str]):
        ...
        if self.vector_db is None:
            ...
            # If it exists, add the new documents to the existing database
            self.vector_db.add_documents(texts)

    def query_with_rag(self, query: str, file_paths: List[str]) -> Optional[str]:
        ...
        # Modify the prompt to list file names and find common recommendations
        prompt = f"""You have the following files loaded: {', '.join([os.path.basename(fp) for fp in file_paths])}\n\n
        The content of these files is:\n\n{context}\n\n
        Please provide a list of the uploaded files and then identify and list any common recommendations found across all the documents."""
...
class ChatbotGUI:
    ...
    def create_widgets(self):
        ...
        self.response_text = scrolledtext.ScrolledText(self.frame, height=20, width=90, wrap=tk.WORD, font=custom_font)
        self.response_text.grid(row=9, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=5)
    ...
    def _process_query_thread(self, query):
        try:
            if self.file_paths:
                ...
                # Query with RAG
                response = self.chatbot.query_with_rag(query, self.file_paths)
            else:
                # Query without RAG
