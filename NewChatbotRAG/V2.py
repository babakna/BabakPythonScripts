# pip install chromadb ollama pypdf2 python-docx openpyxl tk tqdm requests
...
class RAGApplication:
    def __init__(self, root):
        self.root = root
        self.root.title("RAG Desktop Assistant - Run Locally")
        self.root.geometry("800x600")
        
        self.llm_options = []
        self.current_llm = "llama2:latest"
    ...
    def create_widgets(self):
        ...
        self.llm_combobox.bind('<<ComboboxSelected>>', self.on_llm_select)
        ...
        self.show_docs_btn = Button(doc_frame, text="Display Uploaded Docs", command=self.display_uploaded_documents)
        self.show_docs_btn.pack(side=LEFT, padx=5)
        
        self.reset_btn = Button(doc_frame, text="Reset Session", command=self.threaded_reset_system, bg="pink")
        ...
        self.run_btn = Button(button_frame, text="Run Query", command=self.threaded_run_query, bg="lightgreen")
        self.run_btn.pack(side=LEFT, padx=5)
        
        self.clear_btn = Button(button_frame, text="Clear Output", command=self.clear_output)
        self.clear_btn.pack(side=LEFT, padx=5)
        
        self.exit_btn = Button(button_frame, text="Exit", command=self.cleanup_and_exit, bg="orange", width=10)
    ...
    def initialize_models(self):
        try:
            ...
            if not models:
                raise ValueError("No models found in Ollama. Install models with 'ollama pull <model>'")

            self.llm_options = sorted(list(set(models)), key=lambda x: x.lower(), reverse=True)
            
            # Fallback to llama2:latest if current model not available
            if self.current_llm not in self.llm_options:
                self.current_llm = "llama2:latest" if "llama2:latest" in self.llm_options else self.llm_options[0]
            ...
            self.update_output("Please ensure Ollama is running and accessible")
    ...
    def run_query(self):
        ...
        self.set_processing(True)
        self.update_output(f"Processing your query using {self.current_llm}... (This may take a moment)")
    ...
    def set_processing(self, state):
        def safe_state_update():
            ...
            self.clear_btn.config(state=state_btn)
    ...
    def test_ollama_connection(self):
        self.current_llm = self.llm_combobox.get()  # Update current_llm when testing connection
        self.set_processing(True)
        try:
            if self.check_ollama_running():
                self.update_output("Ollama connection successful!")
                self.initialize_models()
            else:
