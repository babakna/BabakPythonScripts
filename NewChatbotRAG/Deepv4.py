...
class RAGApplication:
    ...
    def upload_documents(self):
        ...
        if file_paths:
            ...
            try:
                self.update_output(f"Processing {len(file_paths)} files...")
                self.progress.start()
                
                for idx, file_path in enumerate(tqdm(file_paths, desc="Processing files")):
                    content = self.read_file(file_path)
                    if content:
                        documents.append(content)
                        metadatas.append({"source": file_path})
                        ids.append(str(idx))
                        self.uploaded_documents.append(os.path.basename(file_path))
                ...
                if documents:
                    ...
                    self.update_output(f"Completed ingestion of {len(documents)} documents")
                    self.update_output("You can now enter your query and click 'Run Query'")
            except Exception as e:
