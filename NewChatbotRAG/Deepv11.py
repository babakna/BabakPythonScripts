...
class RAGApplication:
    ...
    def load_available_models(self):
        try:
            ...
            # Extract base model names with fallback
            self.llm_options = sorted(
                list(set(
                    [model['name'].split(':')[0].strip() for model in models if 'name' in model]
                    + [model['name'].strip() for model in models if 'name' in model]
                )),
                key=lambda x: x.lower()
            )

            if not self.llm_options:
                self.llm_options = ["llama2"]
                self.update_output("Warning: No models found in Ollama, using default")
            
            self.llm_combobox['values'] = self.llm_options
            if 'llama2' in self.llm_options:
                self.llm_combobox.set('llama2')
            elif self.llm_options:
                self.llm_combobox.set(self.llm_options[0])
                
            self.update_output(f"Loaded {len(self.llm_options)} models from Ollama")
            
        except Exception as e:
            self.llm_options = ["llama2"]
            self.llm_combobox['values'] = self.llm_options
            self.llm_combobox.set('llama2')
            self.update_output(f"Error loading models: {str(e)}")
    ...
    def create_widgets(self):
        ...
        self.llm_combobox = ttk.Combobox(model_frame, state="readonly")
        ...
        refresh_btn = Button(model_frame, text="Refresh Models", command=self.threaded_refresh_models)
        ...
        self.reset_btn = Button(doc_frame, text="Reset For New Query", command=self.threaded_reset_system)
    ...
    def threaded_refresh_models(self):
        threading.Thread(target=self.load_available_models).start()
    ...
    def reset_system(self):
        ...
        self.initialize_chroma()
        self.update_output("System reset completed. Ready for new query.")
