...
# pip install langchain chromadb tiktoken unstructured[pdf,docx,xlsx] pdfminer.six python-docx openpyxl tkinter requests
...
class ChatbotGUI:
    ...
    def create_widgets(self):
        ...
        self.submit_button = tk.Button(self.frame, text="Submit", command=self.process_query, bg="light green", font=custom_font)
        ...
        self.test_button = tk.Button(self.button_frame, text="Test Connection", command=self.test_connection, bg="light blue", font=custom_font)
    ...
    def test_connection(self):
        try:
            ...
            if response.status_code == 200:
                self.response_text.insert(tk.END, "Connection to Ollama server is successful.\n")
            else:
                self.response_text.insert(tk.END, f"Failed to connect to Ollama server. Status code: {response.status_code}\n")
        except requests.exceptions.RequestException as e:
            self.response_text.insert(tk.END, f"Failed to connect to Ollama server: {e}\n")
    ...
    def start_ollama_serve(self):
        try:
            ...
            self.response_text.insert(tk.END, "Started Ollama server. Please wait a moment and try again.\n")
        except Exception as e:
            self.response_text.insert(tk.END, f"Failed to start Ollama server: {e}\n")
        self.response_text.see(tk.END)
