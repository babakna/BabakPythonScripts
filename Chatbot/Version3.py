...
class ChatbotGUI:
    def __init__(self, master):
        ...
        master.geometry("800x700")  # Increased height to accommodate all widgets
    ...
    def create_widgets(self):
        ...
        self.followup_label = ttk.Label(self.frame, text="Enter any follow up questions:", font=custom_font)
        self.followup_label.grid(row=3, column=0, sticky=tk.W, pady=5)

        self.followup_entry = ttk.Entry(self.frame, width=70, font=custom_font)
        self.followup_entry.grid(row=4, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=5)

        self.submit_button = ttk.Button(self.frame, text="Submit", command=self.process_query, style='Custom.TButton')
        self.submit_button.grid(row=4, column=2, sticky=tk.E, pady=5)

        self.status_label = ttk.Label(self.frame, text="Status:", font=custom_font)
        self.status_label.grid(row=5, column=0, sticky=tk.W, pady=5)
        ...
        self.response_label.grid(row=7, column=0, sticky=tk.W, pady=5)

        self.response_text = scrolledtext.ScrolledText(self.frame, height=15, width=80, wrap=tk.WORD, font=custom_font)
        self.response_text.grid(row=8, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=5)

        # Button row
        self.button_frame = ttk.Frame(self.frame)
        self.button_frame.grid(row=9, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=10)

        self.clear_button = ttk.Button(self.button_frame, text="Clear", command=self.clear_output, style='Custom.TButton')
        self.clear_button.grid(row=0, column=0, padx=5)

        self.save_button = ttk.Button(self.button_frame, text="Save", command=self.save_output, style='Custom.TButton')
        self.save_button.grid(row=0, column=1, padx=5)

        self.test_button = ttk.Button(self.button_frame, text="Test", command=self.test_connection, style='Custom.TButton')
        self.test_button.grid(row=0, column=3, padx=5)

        self.exit_button = ttk.Button(self.button_frame, text="Exit", command=self.exit_application, style='Custom.TButton')
    ...
    def process_query(self):
        ...
        if followup_query and self.current_context:
            query = followup_query  # Use the follow-up query if it exists and context is available
    ...
    def _process_query_thread(self, query):
        try:
            if not self.current_context:
                ...
                response = self.chatbot.query_ollama(query, stream=True)
    ...
    def save_output(self):
        with open("chatbot_output.txt", "w") as f:
            ...
            f.write(self.followup_entry.get() + "\n\n")
            f.write("Status:\n")
