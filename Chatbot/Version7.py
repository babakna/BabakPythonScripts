...
class ChatbotGUI:
    def __init__(self, master):
        ...
        master.geometry("800x800")  # Increased height to accommodate all widgets
    ...
    def create_widgets(self):
        ...
        self.query_entry = tk.Text(self.frame, height=3, width=70, wrap=tk.WORD, font=custom_font)
        ...
        self.followup_entry = tk.Text(self.frame, height=3, width=70, wrap=tk.WORD, font=custom_font)
    ...
    def _process_query_thread(self, query):
        try:
            if not self.current_context:
                ...
                # Pass the conversation history as context for follow-up queries
                context = [msg.content for msg in self.chatbot.conversation_history]
                response = self.chatbot.query_ollama(query, stream=True, context=context)

            if response:
                self.response_text.delete("1.0", tk.END)
                for chunk in response:
                    if self.stop_rendering:  # Stop rendering if flag is set
                        break
                ...
                if not self.stop_rendering:
                    ...
                    self.chatbot.add_to_history("user", query)  # Add user query to history
                    self.chatbot.add_to_history("assistant", self.response_text.get("1.0", tk.END).strip())  # Add assistant response to history
            else:
