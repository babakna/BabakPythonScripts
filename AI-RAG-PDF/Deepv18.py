...
# Ollama runnign locally
# Loads/parses PDFs and populates ChromaDB
# ALlows RAG queries
# Allows limited parameter settings
...
class RAGApplication:
    def __init__(self, root):
        self.root = root
        self.root.title("Babak PDF RAG Tool")
    ...
    def setup_gui(self):
        ...
        style.configure('DarkGreen.TButton', background='green', foreground='black')
        ...
        ttk.Button(left_buttons, text="Process PDFs", command=self.start_processing, style='Green.TButton').pack(side=tk.LEFT, padx=2)
        
        # Right side buttons
        right_buttons = ttk.Frame(button_frame)
        right_buttons.pack(side=tk.RIGHT)
        ttk.Button(right_buttons, text="Stop", command=self.stop_processing, style='Pink.TButton').pack(side=tk.LEFT, padx=2)
        ...
        ttk.Button(right_buttons, text="Exit", command=self.exit_application, style='Red.TButton').pack(side=tk.LEFT, padx=2)
        ...
        # Query entry with scrollbar
        query_frame = ttk.Frame(query_section)
        query_frame.pack(fill=tk.BOTH, expand=True)
        
        self.query_entry = tk.Text(query_frame, height=3, wrap=tk.WORD)
        self.query_entry.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        
        query_scrollbar = ttk.Scrollbar(query_frame, orient=tk.VERTICAL, command=self.query_entry.yview)
        query_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        self.query_entry.config(yscrollcommand=query_scrollbar.set)
        
        ttk.Button(query_section, text="Submit Query", command=self.process_query, style='DarkGreen.TButton').pack(anchor=tk.E, padx=5, pady=5)
