...
def log_message(message):
    """Log messages to the GUI text display in a thread-safe manner."""
    def update_log():
...
def create_backup(destination):
    ...
    # Function to add star and schedule next
    def add_star():
        log_message("*")
        # Check if backup thread is still alive before scheduling next star
    ...
    try:
        ...
        log_message(f"Backup completed on {end_time.strftime('%Y-%m-%d at %I:%M %p')}")
